namespace Lattice.Core.Repositories.SqlServer.Implementations
{
    ...
    /// <summary>
    /// SQL Server implementation of request history methods.
    /// </summary>
    internal class RequestHistoryMethods : IRequestHistoryMethods
    {
        ...
        internal RequestHistoryMethods(SqlServerRepository repo)
        {
        ...
        private static string GetBucketExpression(string interval)
        {
            ...
            {
                "minute" => "DATEADD(minute, DATEDIFF(minute, 0, [createdutc]), 0)",
                "15minute" => "DATEADD(minute, (DATEDIFF(minute, 0, [createdutc]) / 15) * 15, 0)",
                "hour" => "DATEADD(hour, DATEDIFF(hour, 0, [createdutc]), 0)",
                "6hour" => "DATEADD(hour, (DATEDIFF(hour, 0, [createdutc]) / 6) * 6, 0)",
                "day" => "DATEADD(day, DATEDIFF(day, 0, [createdutc]), 0)",
                _ => "DATEADD(hour, DATEDIFF(hour, 0, [createdutc]), 0)"
            };
        }
