namespace Lattice.Core.Repositories.SqlServer.Implementations
{
    ...
    internal class SchemaMethods : ISchemaMethods
    {
        ...
        internal SchemaMethods(SqlServerRepository repo)
        {
        ...
        public async Task<Schema> Create(Schema schema, CancellationToken token = default)
        {
            ...
            string query = $@"
                ...
                VALUES ('{Sanitizer.Sanitize(schema.Id)}',
                        {(schema.Name != null ? $"N'{Sanitizer.Sanitize(schema.Name)}'" : "NULL")},
        ...
        public async Task<Schema> Update(Schema schema, CancellationToken token = default)
        {
            ...
            string query = $@"
                UPDATE [schemas] SET
                    [name] = {(schema.Name != null ? $"N'{Sanitizer.Sanitize(schema.Name)}'" : "NULL")},
