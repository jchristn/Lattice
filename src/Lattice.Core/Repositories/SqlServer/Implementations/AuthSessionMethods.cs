namespace Lattice.Core.Repositories.SqlServer.Implementations
{
    ...
    /// <summary>
    /// SQL Server implementation of authentication session methods.
    /// </summary>
    internal class AuthSessionMethods : IAuthSessionMethods
    {
        private readonly SqlServerRepository _Repo;

        internal AuthSessionMethods(SqlServerRepository repo)
        {
        ...
        public async Task<AuthSession> Create(AuthSession session, CancellationToken token = default)
        {
            ...
            string query = $@"
                ...
                VALUES ('{Sanitizer.Sanitize(session.Id)}',
                        ...
                        {(session.SourceIp != null ? $"N'{Sanitizer.Sanitize(session.SourceIp)}'" : "NULL")},
                        {(session.UserAgent != null ? $"N'{Sanitizer.Sanitize(session.UserAgent)}'" : "NULL")},
                        ...
                        {(session.RevocationReason != null ? $"N'{Sanitizer.Sanitize(session.RevocationReason)}'" : "NULL")},
                        {(session.Active ? 1 : 0)},
        ...
        public async Task<AuthSession> ReadByTokenId(string tokenId, CancellationToken token = default)
        {
            ...
            string query = $"SELECT TOP 1 * FROM [authsessions] WHERE [tokenid] = '{Sanitizer.Sanitize(tokenId)}';";
            DataTable result = await _Repo.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        public async Task<AuthSession> Update(AuthSession session, CancellationToken token = default)
        {
            ...
            string query = $@"
                UPDATE [authsessions] SET
                    ...
                    [sourceip] = {(session.SourceIp != null ? $"N'{Sanitizer.Sanitize(session.SourceIp)}'" : "NULL")},
                    [useragent] = {(session.UserAgent != null ? $"N'{Sanitizer.Sanitize(session.UserAgent)}'" : "NULL")},
                    ...
                    [revocationreason] = {(session.RevocationReason != null ? $"N'{Sanitizer.Sanitize(session.RevocationReason)}'" : "NULL")},
