namespace Lattice.Core.Repositories.SqlServer.Implementations
{
    ...
    /// <summary>
    /// SQL Server implementation of audit methods.
    /// </summary>
    internal class AuditMethods : IAuditMethods
    {
        private readonly SqlServerRepository _Repo;

        internal AuditMethods(SqlServerRepository repo)
        {
        ...
        public async Task<AuditEntry> Create(AuditEntry entry, CancellationToken token = default)
        {
            ...
            string query = $@"
                ...
                VALUES ('{Sanitizer.Sanitize(entry.Id)}',
                        ...
                        {(entry.EventType != null ? $"N'{Sanitizer.Sanitize(entry.EventType)}'" : "NULL")},
                        ...
                        {(entry.RequestType != null ? $"N'{Sanitizer.Sanitize(entry.RequestType)}'" : "NULL")},
                        {(entry.Method != null ? $"N'{Sanitizer.Sanitize(entry.Method)}'" : "NULL")},
                        {(entry.Path != null ? $"N'{Sanitizer.Sanitize(entry.Path)}'" : "NULL")},
                        {(entry.SourceIp != null ? $"N'{Sanitizer.Sanitize(entry.SourceIp)}'" : "NULL")},
                        {(entry.AuthResult != null ? $"N'{Sanitizer.Sanitize(entry.AuthResult)}'" : "NULL")},
                        {(entry.AuthzResult != null ? $"N'{Sanitizer.Sanitize(entry.AuthzResult)}'" : "NULL")},
                        {(entry.DenialReason != null ? $"N'{Sanitizer.Sanitize(entry.DenialReason)}'" : "NULL")},
                        {(entry.BypassReason != null ? $"N'{Sanitizer.Sanitize(entry.BypassReason)}'" : "NULL")},
                        {(entry.RequiredPermission != null ? $"N'{Sanitizer.Sanitize(entry.RequiredPermission)}'" : "NULL")},
                        {entry.ResponseCode},
        ...
        public async Task<List<AuditEntry>> Search(string tenantId, string eventType, DateTime? fromUtc, DateTime? toUtc, int skip, int maxResults, CancellationToken token = default)
        {
            ...
            string query = $"SELECT * FROM [audit]{whereClause} ORDER BY [createdutc] DESC OFFSET {skip} ROWS FETCH NEXT {maxResults} ROWS ONLY;";
            DataTable result = await _Repo.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        private static string BuildWhereClause(string tenantId, string eventType, DateTime? fromUtc, DateTime? toUtc)
        {
            ...
            if (!string.IsNullOrWhiteSpace(eventType))
                conditions.Add($"[eventtype] = N'{Sanitizer.Sanitize(eventType)}'");
