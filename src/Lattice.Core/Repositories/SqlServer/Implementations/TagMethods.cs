namespace Lattice.Core.Repositories.SqlServer.Implementations
{
    ...
    internal class TagMethods : ITagMethods
    {
        ...
        internal TagMethods(SqlServerRepository repo)
        {
        ...
        public async Task<Tag> Create(Tag tag, CancellationToken token = default)
        {
            ...
            string query = $@"
                ...
                VALUES ('{Sanitizer.Sanitize(tag.Id)}',
                        ...
                        N'{Sanitizer.Sanitize(tag.Key)}',
                        {(tag.Value != null ? $"N'{Sanitizer.Sanitize(tag.Value)}'" : "NULL")},
        ...
        public async IAsyncEnumerable<string> FindDocumentIdsByTag(string key, string value, [EnumeratorCancellation] CancellationToken token = default)
        {
            ...
            string whereClause = value != null
                ? $"[key] = N'{Sanitizer.Sanitize(key)}' AND [value] = N'{Sanitizer.Sanitize(value)}'"
                : $"[key] = N'{Sanitizer.Sanitize(key)}'";
        ...
        public async Task<HashSet<string>> FindDocumentIdsByTags(Dictionary<string, string> tags, CancellationToken token = default)
        {
            ...
            if (tagList.Count == 1)
            {
                ...
                string whereClause = tag.Value != null
                    ? $"[key] = N'{Sanitizer.Sanitize(tag.Key)}' AND [value] = N'{Sanitizer.Sanitize(tag.Value)}'"
                    : $"[key] = N'{Sanitizer.Sanitize(tag.Key)}'";
            ...
            else
            {
                ...
                for (int i = 0; i < tagList.Count; i++)
                {
                    ...
                    if (tag.Value != null)
                    {
                        sb.Append($"(t{i}.[key] = N'{Sanitizer.Sanitize(tag.Key)}' AND t{i}.[value] = N'{Sanitizer.Sanitize(tag.Value)}')");
                    }
                    else
                    {
                        sb.Append($"t{i}.[key] = N'{Sanitizer.Sanitize(tag.Key)}'");
                    }
                }
                sb.Append(";");
