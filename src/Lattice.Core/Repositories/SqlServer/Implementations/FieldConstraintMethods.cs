namespace Lattice.Core.Repositories.SqlServer.Implementations
{
    ...
    internal class FieldConstraintMethods : IFieldConstraintMethods
    {
        ...
        internal FieldConstraintMethods(SqlServerRepository repo)
        {
        ...
        public async Task<List<FieldConstraint>> CreateMany(List<FieldConstraint> constraints, CancellationToken token = default)
        {
            ...
            foreach (FieldConstraint constraint in constraints)
            {
                string allowedValuesJson = constraint.AllowedValues != null && constraint.AllowedValues.Count > 0
                    ? $"N'{Sanitizer.Sanitize(System.Text.Json.JsonSerializer.Serialize(constraint.AllowedValues))}'"
                    : "NULL";
                ...
                statements.Add($@"
                    ...
                    VALUES ('{Sanitizer.Sanitize(constraint.Id)}',
                            ...
                            N'{Sanitizer.Sanitize(constraint.FieldPath)}',
                            {(constraint.DataType != null ? $"N'{Sanitizer.Sanitize(constraint.DataType)}'" : "NULL")},
                            {(constraint.Required ? 1 : 0)},
                            {(constraint.Nullable ? 1 : 0)},
                            {(constraint.RegexPattern != null ? $"N'{Sanitizer.Sanitize(constraint.RegexPattern)}'" : "NULL")},
                            ...
                            {allowedValuesJson},
                            {(constraint.ArrayElementType != null ? $"N'{Sanitizer.Sanitize(constraint.ArrayElementType)}'" : "NULL")},
