namespace Lattice.Core.Repositories.Sqlite.Implementations
{
    ...
    /// <summary>
    /// SQLite implementation of authentication session methods.
    /// </summary>
    internal class AuthSessionMethods : IAuthSessionMethods
    {
        private readonly SqliteRepository _Repo;

        internal AuthSessionMethods(SqliteRepository repo)
        {
        ...
        public async Task<AuthSession> Create(AuthSession session, CancellationToken token = default)
        {
            ...
            string query = $@"
                ...
                SELECT * FROM authsessions WHERE id = '{Sanitizer.Sanitize(session.Id)}';
            ";
