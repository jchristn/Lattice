namespace Lattice.Core.Repositories.Mysql.Implementations
{
    ...
    /// <summary>
    /// MySQL implementation of document methods.
    /// </summary>
    internal class DocumentMethods : IDocumentMethods
    {
        private readonly MysqlRepository _Repo;

        internal DocumentMethods(MysqlRepository repo)
        {
        ...
        public async Task<Document> Create(Document document, CancellationToken token = default)
        {
            ...
            string selectQuery = $"SELECT * FROM `documents` WHERE `id` = '{Sanitizer.Sanitize(document.Id)}';";
            ...
            [EnumeratorCancellation] CancellationToken token = default)
        {
            ...
            // MySQL uses LIMIT with large number for no limit, or we can use a very large number
            string query = $@"
