namespace Lattice.Core.Repositories.Mysql.Implementations
{
    ...
    /// <summary>
    /// MySQL implementation of schema element methods.
    /// </summary>
    internal class SchemaElementMethods : ISchemaElementMethods
    {
        private readonly MysqlRepository _Repo;

        internal SchemaElementMethods(MysqlRepository repo)
        {
        ...
        public async Task<SchemaElement> Create(SchemaElement element, CancellationToken token = default)
        {
            ...
            string selectQuery = $"SELECT * FROM `schemaelements` WHERE `id` = '{Sanitizer.Sanitize(element.Id)}';";
