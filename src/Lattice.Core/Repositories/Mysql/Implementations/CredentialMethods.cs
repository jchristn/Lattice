namespace Lattice.Core.Repositories.Mysql.Implementations
{
    ...
    internal class CredentialMethods : ICredentialMethods
    {
        ...
        internal CredentialMethods(MysqlRepository repo)
        {
        ...
        public async Task<Credential> Create(Credential credential, CancellationToken token = default)
        {
            ...
            string selectQuery = $"SELECT * FROM `credentials` WHERE `id` = '{Sanitizer.Sanitize(credential.Id)}';";
        ...
        public async Task<Credential> Update(Credential credential, CancellationToken token = default)
        {
            ...
            string query = $@"
                UPDATE `credentials` SET
                    ...
                    `active` = {(credential.Active ? 1 : 0)},
                    `isprotected` = {(credential.IsProtected ? 1 : 0)},
