namespace Lattice.Core.Repositories.Mysql.Implementations
{
    ...
    /// <summary>
    /// MySQL implementation of request history methods.
    /// </summary>
    internal class RequestHistoryMethods : IRequestHistoryMethods
    {
        ...
        internal RequestHistoryMethods(MysqlRepository repo)
        {
        ...
        private static string GetBucketExpression(string interval)
        {
            ...
            {
                "minute" => "DATE_ADD(DATE(`createdutc`), INTERVAL (HOUR(`createdutc`) * 60 + MINUTE(`createdutc`)) MINUTE)",
                "15minute" => "DATE_ADD(DATE(`createdutc`), INTERVAL (HOUR(`createdutc`) * 60 + FLOOR(MINUTE(`createdutc`) / 15) * 15) MINUTE)",
                "hour" => "DATE_ADD(DATE(`createdutc`), INTERVAL HOUR(`createdutc`) HOUR)",
                "6hour" => "DATE_ADD(DATE(`createdutc`), INTERVAL (FLOOR(HOUR(`createdutc`) / 6) * 6) HOUR)",
                "day" => "DATE(`createdutc`)",
                _ => "DATE_ADD(DATE(`createdutc`), INTERVAL HOUR(`createdutc`) HOUR)"
            };
        }
