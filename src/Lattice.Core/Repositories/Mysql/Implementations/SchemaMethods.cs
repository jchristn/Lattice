namespace Lattice.Core.Repositories.Mysql.Implementations
{
    ...
    /// <summary>
    /// MySQL implementation of schema methods.
    /// </summary>
    internal class SchemaMethods : ISchemaMethods
    {
        private readonly MysqlRepository _Repo;

        internal SchemaMethods(MysqlRepository repo)
        {
        ...
        public async Task<Schema> Create(Schema schema, CancellationToken token = default)
        {
            ...
            string selectQuery = $"SELECT * FROM `schemas` WHERE `id` = '{Sanitizer.Sanitize(schema.Id)}';";
