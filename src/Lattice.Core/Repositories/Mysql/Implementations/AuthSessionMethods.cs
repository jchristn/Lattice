namespace Lattice.Core.Repositories.Mysql.Implementations
{
    ...
    /// <summary>
    /// MySQL implementation of authentication session methods.
    /// </summary>
    internal class AuthSessionMethods : IAuthSessionMethods
    {
        private readonly MysqlRepository _Repo;

        internal AuthSessionMethods(MysqlRepository repo)
        {
        ...
        public async Task<AuthSession> Create(AuthSession session, CancellationToken token = default)
        {
            ...
            string selectQuery = $"SELECT * FROM `authsessions` WHERE `id` = '{Sanitizer.Sanitize(session.Id)}';";
