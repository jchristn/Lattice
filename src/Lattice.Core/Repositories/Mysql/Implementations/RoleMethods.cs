namespace Lattice.Core.Repositories.Mysql.Implementations
{
    ...
    /// <summary>
    /// MySQL implementation of role, permission, and assignment methods.
    /// </summary>
    internal class RoleMethods : IRoleMethods
    {
        private readonly MysqlRepository _Repo;

        internal RoleMethods(MysqlRepository repo)
        {
        ...
        public async Task<UserRole> CreateRole(UserRole role, CancellationToken token = default)
        {
            ...
            string selectQuery = $"SELECT * FROM `userroles` WHERE `id` = '{Sanitizer.Sanitize(role.Id)}';";
            DataTable result = await _Repo.ExecuteQueryAsync(selectQuery, false, token).ConfigureAwait(false);
        ...
        public async Task<UserRole> ReadRoleByName(string tenantId, string name, CancellationToken token = default)
        {
            ...
            string query = $"SELECT * FROM `userroles` WHERE {tenantClause} AND `name` = '{Sanitizer.Sanitize(name)}' ORDER BY `tenantid` DESC LIMIT 1;";
            DataTable result = await _Repo.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        public async Task<UserRole> UpdateRole(UserRole role, CancellationToken token = default)
        {
            ...
            string query = $@"
                UPDATE `userroles` SET
                    `name` = '{Sanitizer.Sanitize(role.Name)}',
        ...
        public async Task<Permission> CreatePermission(Permission permission, CancellationToken token = default)
        {
            ...
            string query = $@"
                ...
                VALUES ('{Sanitizer.Sanitize(permission.Id)}',
                        ...
                        {(permission.Name != null ? $"'{Sanitizer.Sanitize(permission.Name)}'" : "NULL")},
                        '{Sanitizer.Sanitize(SerializeEnumNames(permission.ResourceTypes))}',
                        '{Sanitizer.Sanitize(SerializeEnumNames(permission.OperationTypes))}',
            ...
            string selectQuery = $"SELECT * FROM `permissions` WHERE `id` = '{Sanitizer.Sanitize(permission.Id)}';";
            DataTable result = await _Repo.ExecuteQueryAsync(selectQuery, false, token).ConfigureAwait(false);
            if (result.Rows.Count > 0)
        ...
        public async Task<Permission> UpdatePermission(Permission permission, CancellationToken token = default)
        {
            ...
            string query = $@"
                UPDATE `permissions` SET
                    `name` = {(permission.Name != null ? $"'{Sanitizer.Sanitize(permission.Name)}'" : "NULL")},
                    `resourcetypes` = '{Sanitizer.Sanitize(SerializeEnumNames(permission.ResourceTypes))}',
                    `operationtypes` = '{Sanitizer.Sanitize(SerializeEnumNames(permission.OperationTypes))}',
        ...
        public async Task<RolePermissionMap> CreateRolePermissionMap(RolePermissionMap map, CancellationToken token = default)
        {
            ...
            string selectQuery = $"SELECT * FROM `rolepermissionmaps` WHERE `id` = '{Sanitizer.Sanitize(map.Id)}';";
            DataTable result = await _Repo.ExecuteQueryAsync(selectQuery, false, token).ConfigureAwait(false);
            if (result.Rows.Count > 0)
        ...
        public async Task<UserRoleAssignment> CreateUserRoleAssignment(UserRoleAssignment assignment, CancellationToken token = default)
        {
            ...
            string query = $@"
                ...
                VALUES ('{Sanitizer.Sanitize(assignment.Id)}',
                        ...
                        {(assignment.RoleName != null ? $"'{Sanitizer.Sanitize(assignment.RoleName)}'" : "NULL")},
            ...
            string selectQuery = $"SELECT * FROM `userroleassignments` WHERE `id` = '{Sanitizer.Sanitize(assignment.Id)}';";
            DataTable result = await _Repo.ExecuteQueryAsync(selectQuery, false, token).ConfigureAwait(false);
            if (result.Rows.Count > 0)
        ...
        public async Task<CredentialScopeAssignment> CreateCredentialScopeAssignment(CredentialScopeAssignment assignment, CancellationToken token = default)
        {
            ...
            string query = $@"
                ...
                VALUES ('{Sanitizer.Sanitize(assignment.Id)}',
                        ...
                        {(int)assignment.ResourceScope},
                        {(assignment.ResourceId != null ? $"'{Sanitizer.Sanitize(assignment.ResourceId)}'" : "NULL")},
                        '{Sanitizer.Sanitize(SerializeEnumNames(assignment.Permissions))}',
                        '{Sanitizer.Sanitize(SerializeEnumNames(assignment.ResourceTypes))}',
                        {(assignment.Active ? 1 : 0)},
                        '{Converters.ToTimestamp(assignment.CreatedUtc)}',
                        '{Converters.ToTimestamp(assignment.LastUpdateUtc)}');
            ";
            ...
            string selectQuery = $"SELECT * FROM `credentialscopeassignments` WHERE `id` = '{Sanitizer.Sanitize(assignment.Id)}';";
            DataTable result = await _Repo.ExecuteQueryAsync(selectQuery, false, token).ConfigureAwait(false);
            if (result.Rows.Count > 0)
