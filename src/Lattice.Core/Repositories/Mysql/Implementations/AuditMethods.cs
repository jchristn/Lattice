namespace Lattice.Core.Repositories.Mysql.Implementations
{
    ...
    /// <summary>
    /// MySQL implementation of audit methods.
    /// </summary>
    internal class AuditMethods : IAuditMethods
    {
        private readonly MysqlRepository _Repo;

        internal AuditMethods(MysqlRepository repo)
        {
        ...
        public async Task<AuditEntry> Create(AuditEntry entry, CancellationToken token = default)
        {
            ...
            string selectQuery = $"SELECT * FROM `audit` WHERE `id` = '{Sanitizer.Sanitize(entry.Id)}';";
