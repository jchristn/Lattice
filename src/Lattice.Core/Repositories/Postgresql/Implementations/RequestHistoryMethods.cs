namespace Lattice.Core.Repositories.Postgresql.Implementations
{
    ...
    /// <summary>
    /// PostgreSQL implementation of request history methods.
    /// </summary>
    internal class RequestHistoryMethods : IRequestHistoryMethods
    {
        ...
        internal RequestHistoryMethods(PostgresqlRepository repo)
        {
        ...
        public async Task<RequestHistorySummaryResult> GetSummary(RequestHistorySearchFilter filter, string interval, CancellationToken token = default)
        {
            ...
            foreach (DataRow row in result.Rows)
            {
                ...
                {
                    TimestampUtc = DateTime.SpecifyKind(Convert.ToDateTime(row["bucketutc"]), DateTimeKind.Utc),
        ...
        private static string GetBucketExpression(string interval)
        {
            ...
            {
                "minute" => "date_trunc('minute', createdutc)",
                "15minute" => "date_trunc('hour', createdutc) + CAST(FLOOR(EXTRACT(MINUTE FROM createdutc) / 15) AS INTEGER) * INTERVAL '15 minute'",
                "hour" => "date_trunc('hour', createdutc)",
                "6hour" => "date_trunc('day', createdutc) + CAST(FLOOR(EXTRACT(HOUR FROM createdutc) / 6) AS INTEGER) * INTERVAL '6 hour'",
                "day" => "date_trunc('day', createdutc)",
                _ => "date_trunc('hour', createdutc)"
            };
        }
